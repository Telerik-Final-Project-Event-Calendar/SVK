...
export default function EventDetails() {
  const { eventId } = useParams();
  const { user } = useContext(AppContext); // assumes you provide user from context
  ...
  useEffect(() => {
    if (user && event?.participants) {
      setHasJoined(event.participants.includes(user.uid));
    }
  }, [user, event]);
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setEvent((prev) =>
        prev
          ? { ...prev, participants: [...(prev.participants || []), user.uid] }
          : prev
      );
    } catch (err) {
